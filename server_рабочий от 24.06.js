...
async function fillPlinthBlock(worksheet, startRow, blockType, plinthData, globalModel) {
    ...
    if (hasReader) {
        ...
        if (blockType === 'input') {
            ...
            // Для выходного блока – проверяем условие замены на "вх"
...
app.post('/generate', async (req, res) => {
    try {
        ...
        // Если не хватает блоков – копируем
        ...
        // Получаем актуальный список всех блоков после заполнения
        const allBlocks = getBlocks(worksheet);
        // Множество startRow используемых блоков
        ...
        // Отбираем блоки для удаления
        const blocksToDelete = allBlocks.filter(b => !usedStartRows.has(b.startRow));
        // Сортируем по убыванию startRow, чтобы удалять с конца
        ...
        for (let i = 0; i < boards.length; i++) {
            ...
            let readerLabel2 = '';
            if (hasReader2) {
                // Проверяем условие замены на "вх" для выходного плинта
