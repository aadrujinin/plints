...
// ---------- Прокси для получения проектов из Аспро.Cloud (использует .env) ----------
app.get('/api/projects', async (req, res) => {
    try {
        const company = process.env.ASPRO_COMPANY;
        const apiKey = process.env.ASPRO_API_KEY;
        if (!company || !apiKey) {
            return res.status(500).json({ error: 'На сервере не заданы ASPRO_COMPANY и ASPRO_API_KEY в .env' });
        }
        const url = `https://${company}.aspro.cloud/api/v1/module/st/projects/list?api_key=${apiKey}`;
        const response = await fetch(url);
        if (!response.ok) {
            const text = await response.text();
            return res.status(response.status).json({ error: `Ошибка Аспро: ${text}` });
        }
        ...
        if (data.response && Array.isArray(data.response.items)) {
            ...
            console.warn('Неизвестный формат ответа Аспро:', data);
            return res.json([]);
        }

        const projects = items.map(p => ({
            id: p.id || p.ID || '',
            name: p.name || p.title || p.Name || 'Без названия'
        })).filter(p => p.id);

        res.json(projects);
    } catch (err) {
        console.error(err);
        res.status(500).json({ error: 'Ошибка при запросе к Аспро: ' + err.message });
    }
});
...
async function fillPlinthBlock(worksheet, startRow, blockType, plinthData, globalModel) {
    ...
    if (hasReader) {
        ...
        if (blockType === 'input') {
            ...
            const exitCable = cn.exit_btn || '';
            const exitNum = parseInt(exitCable, 10);
            if (hasExit && exitNum > 1) {
                deviceLabel = 'Считыватель вх';
            } else {
...
app.post('/generate', async (req, res) => {
    try {
        ...
        for (let i = 0; i < boards.length; i++) {
            ...
            const exitCable2 = cn2.exit_btn || '';
            const exitNum2 = parseInt(exitCable2, 10);

            let readerLabel2 = '';
            if (hasReader2) {
                if (hasExit2 && exitNum2 > 1) {
