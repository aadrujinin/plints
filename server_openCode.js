...
app.get('/api/aspro/tasks', async (req, res) => {
    ...
    try {
        ...
        // Загружаем стадии проектов для сопоставления project_stage_id -> проект
        ...
        try {
            ...
            if (stagesRes.ok) {
                ...
                console.log(`[aspro-tasks] Загружено ${Object.keys(stageToProject).length} стадий проектов`);
            }
            ...
            if (Object.keys(stageToProject).length === 0) {
                ...
                if (altRes.ok) {
                    ...
                    console.log(`[aspro-tasks] Загружено ${Object.keys(stageToProject).length} стадий (альт)`);
                }
            }
        } catch (e) {
        ...
        // Для каждой задачи пытаемся получить детальную информацию с проектом
        const enriched = [];
        for (const task of filtered) {
            ...
            enriched.push({ ...task, _project_name: projectName, _project_id: projectId });
        }
