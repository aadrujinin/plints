...
async function fillPlinthBlock(worksheet, startRow, blockType, plinthData, globalModel) {
    ...
    // Плинтодержатель – только группа по 15
    ...
    if (hasReader) {
        ...
        if (blockType === 'input') {
            ...
            // выходной плинт – если есть кнопка выхода, то считыватель входной
    ...
    // --- ЗАМОК / ЗАМОК ПОЖ. (исправлено: убран поворот текста) ---
    ...
    if (hasLock) {
        ...
        // Убрано: cellJ.alignment = { textRotation: 255 };
...
app.post('/generate', async (req, res) => {
    try {
        ...
        const allBlocks = getBlocks(worksheet);
        ...
        console.log(`🗑️ Удалено ${blocksToDelete.length} неиспользуемых блоков`);

        // --- УДАЛЕНИЕ ПУСТЫХ СТРОК ПОСЛЕ ПОСЛЕДНЕГО БЛОКА ---
        const remainingBlocks = getBlocks(worksheet);
        if (remainingBlocks.length > 0) {
            ...
            if (totalRows > lastEndRow) {
                ...
                console.log(`🧹 Удалены пустые строки после строки ${lastEndRow}`);
            }
        }
        ...
        for (let i = 0; i < boards.length; i++) {
            const board = boards[i];
            const boardNumber = i + 1;
            const room1 = board.plinth1.room || '';
            const room2 = board.plinth2.room || '';

            const num1 = i * 2 + 1; // нечётный – входной
            const num2 = i * 2 + 2; // чётный – выходной
            ...
            if (hasReader1) {
                ...
                const gCell = cheatSheet.getCell(`G${rowIdx}`);
                gCell.value = counter++;
                // Входной – всегда красный
                gCell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFFF0000' } };
            } else {
                cheatSheet.getCell(`D${rowIdx}`).value = '';
                cheatSheet.getCell(`F${rowIdx}`).value = '';
                cheatSheet.getCell(`G${rowIdx}`).value = '';
            }
            cheatSheet.getCell(`E${rowIdx}`).value = `пом. ${room1}`;
            rowIdx++;
            ...
            const hasExit2 = Object.values(tm2).includes('exit_btn');
            ...
            if (hasReader2) {
                let readerLabel2 = '';
                if (hasExit2) {
                    readerLabel2 = 'Считыватель вх';
                } else {
                    readerLabel2 = 'Считыватель вых';
                }
                cheatSheet.getCell(`D${rowIdx}`).value = readerLabel2;
                cheatSheet.getCell(`F${rowIdx}`).value = `ШЛ.${cn2.reader || ''}`;
                const gCell = cheatSheet.getCell(`G${rowIdx}`);
                gCell.value = counter++;
                if (readerLabel2 === 'Считыватель вх') {
                    gCell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFFFA500' } }; // оранжевый
                } else {
                    gCell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFFF0000' } }; // красный
                }
            } else {
                cheatSheet.getCell(`D${rowIdx}`).value = '';
                cheatSheet.getCell(`F${rowIdx}`).value = '';
            ...
            cheatSheet.getCell(`E${rowIdx}`).value = `пом. ${room2}`;
            rowIdx++;
        ...
        // Лист "Шпора общая" – копия значений из "шпоргалка" (без заливки)
